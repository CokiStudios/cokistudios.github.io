...
export async function initGlobalI18n() {
    try {
        ...
        //  1. Verificar si hay usuario conectado y obtener su idioma de Supabase
        try {
        ...
        console.warn(' No se pudo cargar translations.json:', e);
    }
}
...
//  Guarda el idioma seleccionado directamente en Supabase (user_metadata)
export async function syncLanguageToSupabase(lang) {
    try {
        ...
        if (user) {
            ...
            if (error) {
                console.warn(' No se pudo sincronizar el idioma en Supabase:', error.message);
            } else {
                console.log(` Idioma '${lang}' guardado en la cuenta CSID (${user.email})`);
            }
        }
    } catch (e) {
